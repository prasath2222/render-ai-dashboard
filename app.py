...
section[data-testid="stSidebar"] * {{ color: {TXT2} !important; font-family: 'Outfit', sans-serif !important; }}
...
.stCheckbox span {{ color: {TXT2} !important; font-size: 13px !important; font-family: 'Outfit', sans-serif !important; }}
...
.m-card:hover {{ border-color: {BORDER2}; transform: translateY(-1px); }}
.m-card-top {{
...
.m-label {{
    ...
    letter-spacing: 0.15em; color: {TXT3};
...
.m-sub {{
    font-family: 'JetBrains Mono', monospace;
    font-size: 10px; color: {TXT2}; margin-top: 6px;
}}
...
.ind-label {{
    font-family: 'JetBrains Mono', monospace;
    font-size: 9px; font-weight: 700;
    letter-spacing: 0.13em; color: {TXT3};
...
.pred-title {{
    ...
    font-weight: 700; letter-spacing: 0.18em; color: {TXT3};
...
.prob-label {{
    ...
    letter-spacing: 0.1em; color: {TXT3};
}}
.prob-val {{
...
def _layout(fig, h=400, title="", margin=None):
    m = margin or dict(l=8, r=8, t=30 if title else 8, b=8)
    fig.update_layout(
        height=h,
        paper_bgcolor=BG2,
        plot_bgcolor=BG2,
        font=dict(family="Outfit", color=TXT2, size=11),
        margin=m,
        title=dict(
            text=title,
            font=dict(family="JetBrains Mono", size=10, color=TXT3),
            x=0.01, y=0.99,
        ) if title else None,
        xaxis=dict(
            gridcolor=BORDER, showgrid=True, zeroline=False, showline=False,
            tickfont=dict(family="JetBrains Mono", size=9, color=TXT3),
            rangeslider=dict(visible=False),
            showspikes=True, spikemode="across", spikesnap="cursor",
            spikecolor=TXT3, spikedash="dot", spikethickness=1,
        ),
        yaxis=dict(
            gridcolor=BORDER, showgrid=True, zeroline=False, showline=False,
            tickfont=dict(family="JetBrains Mono", size=9, color=TXT3),
        ...
        legend=dict(
            ...
            font=dict(family="Outfit", size=10, color=TXT2),
...
def apply_spike(fig, rows=1):
    for i in range(1, rows + 1):
        fig.update_xaxes(showspikes=True, spikemode="across", spikecolor=TXT3,
                         spikedash="dot", spikethickness=1, row=i, col=1)
        fig.update_yaxes(showspikes=True, spikemode="across", spikecolor=TXT3,
                         spikedash="dot", spikethickness=1, row=i, col=1)
    return fig
...
def metric_card(label, value, sub, color):
    return f"""
    <div class="m-card">
      <div class="m-card-top" style="background:{color};"></div>
...
def chart_candles(df, show_ema, show_bb, show_vwap, show_vol):
    ...
    if show_bb and "BB_U" in df.columns:
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=BORDER2, width=0.8), hoverinfo="skip",
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df.index, y=df["BB_L"].values, name="BB Lower",
            line=dict(color=BORDER2, width=0.8),
            fill="tonexty", fillcolor=rgba(BLUE, 0.06),
    ...
    for i in range(1, rows + 1):
        fig.update_xaxes(showgrid=True, gridcolor=BORDER,
                         showspikes=True, spikemode="across", spikecolor=TXT3,
                         spikedash="solid", spikethickness=1, row=i, col=1)
...
def chart_macd(df):
    ...
    _layout(fig, h=240, title="MACD  (12 · 26 · 9)")
    apply_spike(fig)
    return fig
...
def chart_rsi(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=ORANGE, width=1.8), name="RSI 14",
        fill="tozeroy", fillcolor=rgba(ORANGE, 0.08),
    ))
    if "RSI7" in df.columns:
        fig.add_trace(go.Scatter(
            x=df.index, y=df["RSI7"].values,
            line=dict(color=CYAN, width=1.1, dash="dot"), name="RSI 7",
        ))
    for lvl, col in [(70, RED), (30, GREEN), (50, BORDER2)]:
        fig.add_hline(y=lvl, line_width=0.6, line_dash="dot", line_color=col)
    _layout(fig, h=240, title="RSI  (14 · 7)")
...
def chart_stoch(df):
    ...
    for lvl, col in [(80, RED), (20, GREEN)]:
        fig.add_hline(y=lvl, line_width=0.6, line_dash="dot", line_color=col)
    _layout(fig, h=240, title="Stochastic  (14 · 3 · 3)")
    fig.update_yaxes(range=[0, 100])
    apply_spike(fig)
    return fig
...
def chart_adx(df):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["ADX"].values,
                              ...
                              fill="tozeroy", fillcolor=rgba(YELLOW, 0.08)))
    ...
    _layout(fig, h=240, title="ADX  — Trend Strength")
    apply_spike(fig)
    return fig
...
def chart_bb(df):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_W"].values, name="BB Width",
                              line=dict(color=PURPLE, width=1.6),
                              fill="tozeroy", fillcolor=rgba(PURPLE, 0.08)), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_P"].values, name="BB %B",
                              line=dict(color=TEAL, width=1.4)), row=2, col=1)
...
def chart_obv(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=BLUE, width=1.6), name="OBV",
        fill="tozeroy", fillcolor=rgba(BLUE, 0.08),
    ))
    _layout(fig, h=240, title="OBV  — On-Balance Volume")
    apply_spike(fig)
    return fig
...
def chart_cmf(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df.index, y=df["CMF"].values,
                         marker_color=cmf_colors, name="CMF", opacity=0.9))
    fig.add_hline(y=0, line_width=0.5, line_color=BORDER2)
    _layout(fig, h=240, title="CMF  — Chaikin Money Flow  (Inflow / Outflow)")
    apply_spike(fig)
    return fig
...
def chart_volume(df):
    vc = [GREEN if float(c_) >= float(o_) else RED
          for c_, o_ in zip(df["Close"].values, df["Open"].values)]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df.index, y=df["Volume"].values,
                         marker_color=vc, name="Volume", opacity=0.8))
    if "VOL_MA" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["VOL_MA"].values,
                                  line=dict(color=ORANGE, width=1.4),
                                  name="Vol MA20"))
    _layout(fig, h=240, title="Volume  — Buy / Sell Bars")
    apply_spike(fig)
    return fig
...
def chart_mfi(df):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df["MFI"].values,
                              line=dict(color=TEAL, width=1.6), name="MFI",
                              fill="tozeroy", fillcolor=rgba(TEAL, 0.08)))
    for lvl, col in [(80, RED), (20, GREEN)]:
        fig.add_hline(y=lvl, line_width=0.6, line_dash="dot", line_color=col)
    _layout(fig, h=240, title="MFI  — Money Flow Index (14)")
    fig.update_yaxes(range=[0, 100])
    apply_spike(fig)
    return fig
...
def chart_cci_williams(df):
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        vertical_spacing=0.04, row_heights=[0.5, 0.5])
    cci_colors = [GREEN if v <= -100 else RED if v >= 100 else TXT3
                  for v in df["CCI"].values]
    fig.add_trace(go.Bar(x=df.index, y=df["CCI"].values,
                         marker_color=cci_colors, name="CCI", opacity=0.8), row=1, col=1)
    for lvl, col in [(100, RED), (-100, GREEN)]:
        fig.add_hline(y=lvl, line_width=0.6, line_dash="dot", line_color=col, row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["WILLIAMS"].values,
                              line=dict(color=PINK, width=1.4), name="Williams %R"), row=2, col=1)
...
def chart_roc(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df.index, y=df["ROC5"].values,
                         marker_color=roc_colors, name="ROC 5", opacity=0.85))
    fig.add_hline(y=0, line_width=0.5, line_color=BORDER2)
    _layout(fig, h=240, title="ROC  — Rate of Change (5)")
    apply_spike(fig)
    return fig


# ═══════════════════════════════════════════════════════
# PREDICTION CARDS HTML
# ═══════════════════════════════════════════════════════
def classification_card_html(cls_sig, cls_proba, conf, sig_col, cur_price, inr_rate):
    ...
    move   = ("UPWARD MOVE EXPECTED" if cls_sig == "BUY"
               else "DOWNWARD MOVE EXPECTED" if cls_sig == "SELL"
               else "SIDEWAYS / NO CLEAR MOVE")

    def prow(lbl, pct, col):
        return f"""
    ...
    return f"""
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:18px;
         padding:26px;border-top:2px solid {sig_col};height:100%;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.18em;color:{TXT3};margin-bottom:18px;">
        ◈ CLASSIFICATION MODEL
        <span style="font-size:8px;color:{TXT3};margin-left:6px;">XGBoost · LightGBM · RandomForest</span>
      </div>
      <div style="display:flex;align-items:flex-end;gap:14px;margin-bottom:6px;">
        <div style="font-family:'Outfit',sans-serif;font-size:56px;font-weight:900;
             color:{sig_col};letter-spacing:-2px;line-height:1;">{cls_sig}</div>
        <div style="font-size:32px;color:{sig_col};margin-bottom:6px;">{arrow}</div>
      </div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{sig_col};
           letter-spacing:0.08em;margin-bottom:4px;">{move}</div>
      <div style="font-family:'Outfit',sans-serif;font-size:13px;color:{TXT2};
           margin-bottom:24px;">Current: <b style="color:#f0f4ff;">${cur_price:.4f}</b>
           &nbsp;·&nbsp; ₹{cur_price*inr_rate:,.2f}</div>
      {prow("BUY",  p_buy,  GREEN)}
      {prow("HOLD", p_hold, YELLOW)}
      {prow("SELL", p_sell, RED)}
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid {BORDER};
           display:flex;justify-content:space-between;align-items:center;">
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};">
          MODEL CONFIDENCE
        </span>
        <span style="font-family:'Outfit',sans-serif;font-size:26px;font-weight:900;
...
def regression_card_html(cur_price, reg_price, inr_rate, atr_v, ts_score):
    ...
    arrow_lbl = "↑ BULLISH FORECAST" if reg_chg >= 0 else "↓ BEARISH FORECAST"
    ...
    def trow(hor, price):
        ...
        return f"""
        <tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
          ...
          <td style="text-align:right;border-top:1px solid {BORDER};padding:10px 0;">
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};">
              ₹{price*inr_rate:,.2f} &nbsp;
    ...
    return f"""
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:18px;
         padding:26px;border-top:2px solid {reg_col};height:100%;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.18em;color:{TXT3};margin-bottom:18px;">
        ◈ REGRESSION FORECAST
        <span style="font-size:8px;color:{TXT3};margin-left:6px;">Price prediction model</span>
      </div>
      <div style="font-family:'Outfit',sans-serif;font-size:42px;font-weight:900;
           color:{reg_col};letter-spacing:-1px;line-height:1;margin-bottom:4px;">${reg_price:.4f}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{TXT2};margin-bottom:4px;">
        ₹{reg_price*inr_rate:,.2f}
      </div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700;
           color:{reg_col};margin-bottom:22px;">{arrow_lbl} &nbsp; {reg_chg:+.2f}%</div>
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr>
            <th style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
                letter-spacing:0.12em;text-align:left;padding-bottom:8px;">HORIZON</th>
            <th style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
                letter-spacing:0.12em;text-align:right;padding-bottom:8px;">TARGET PRICE</th>
          </tr>
        </thead>
        <tbody>
          {trow("1 Hour",   r1h)}
          {trow("4 Hours",  r4h)}
          {trow("24 Hours", reg_price)}
        </tbody>
      </table>
      <div style="margin-top:16px;padding-top:12px;border-top:1px solid {BORDER};">
        <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};margin-bottom:5px;">
          ±1σ CONFIDENCE BAND
        </div>
        <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{TXT2};">
...
def regime_html(score, atr_v, cur_price):
    clamped = max(-100, min(100, score))
    pct     = int((clamped + 100) / 2)
    color   = GREEN if clamped >= 30 else RED if clamped <= -30 else YELLOW
    label_  = "BULL MARKET" if clamped >= 30 else "BEAR MARKET" if clamped <= -30 else "SIDEWAYS"
    trend_s = "Strong" if abs(clamped) > 60 else "Moderate" if abs(clamped) > 30 else "Weak"
    vol_r   = atr_v / max(cur_price, 1e-9) * 100
    vol_s   = "High" if vol_r > 3 else "Normal" if vol_r > 1 else "Low"
    struct  = "Bullish" if clamped >= 30 else "Bearish" if clamped <= -30 else "Ranging"
    brkout  = "Confirmed" if abs(clamped) > 50 else "Possible" if abs(clamped) > 30 else "None"
    mom     = ("Strong Bull" if clamped > 60 else "Bullish" if clamped > 30
               else "Strong Bear" if clamped < -60 else "Bearish" if clamped < -30 else "Neutral")

    def rrow(lbl, val, col):
        return f"""<tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
               padding:9px 0;border-top:1px solid {BORDER};">{lbl}</td>
          <td style="text-align:right;font-family:'Outfit',sans-serif;font-size:12px;
               font-weight:700;color:{col};border-top:1px solid {BORDER};padding:9px 0;">{val}</td>
        </tr>"""

    return f"""
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:18px;
         padding:26px;border-top:2px solid {color};height:100%;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.18em;color:{TXT3};margin-bottom:18px;">◈ MARKET REGIME</div>
      ...
      <div style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;
           color:{color};letter-spacing:0.1em;margin:8px 0 20px;">{label_}</div>
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:18px;">
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{RED};">BEAR</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{YELLOW};">NEUTRAL</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{GREEN};">BULL</span>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        {rrow("TREND STRENGTH", trend_s, GREEN if trend_s=="Strong" else YELLOW)}
        {rrow("VOLATILITY",     vol_s,   ORANGE)}
        {rrow("STRUCTURE",      struct,  GREEN if struct=="Bullish" else RED if struct=="Bearish" else YELLOW)}
        {rrow("BREAKOUT",       brkout,  GREEN if brkout=="Confirmed" else YELLOW)}
        {rrow("MOMENTUM",       mom,     GREEN if "Bull" in mom else RED if "Bear" in mom else YELLOW)}
      </table>
    </div>"""
...
def sr_table_html(df, cur_price, inr_rate, atr_v):
    ...
    def row(lbl, price, n, col):
        return f"""<tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{col};
          ...
          <td style="text-align:center;border-top:1px solid {BORDER};padding:10px 0;">
            <div style="font-family:'JetBrains Mono',monospace;font-size:13px;
                 color:#f0f4ff;font-weight:700;">${price:.4f}</div>
          </td>
          <td style="text-align:center;border-top:1px solid {BORDER};padding:10px 0;">
            <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{TXT2};">
              ₹{price*inr_rate:,.2f}
          ...
          <td style="text-align:right;border-top:1px solid {BORDER};padding:10px 0;">{dots(n,col)}</td>
        </tr>"""

    return f"""
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:16px;padding:22px;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.16em;color:{TXT3};margin-bottom:14px;">◈ SUPPORT & RESISTANCE LEVELS</div>
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr>
            <th style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
        ...
        <tbody>
          {row("RESISTANCE 3", r3, 3, RED)}
          {row("RESISTANCE 2", r1, 4, RED)}
          {row("RESISTANCE 1", r2, 5, RED)}
          <tr>
          ...
          {row("SUPPORT 1",  s2, 5, GREEN)}
          {row("SUPPORT 2",  s1, 4, GREEN)}
          {row("SUPPORT 3",  s3, 3, GREEN)}
        </tbody>
      </table>
    </div>"""


def trading_setup_html(cur_price, inr_rate, atr_v, signal):
    ...
    def srow(lbl, usd, col):
        return f"""<tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
               padding:11px 0;border-top:1px solid {BORDER};">{lbl}</td>
          <td style="text-align:right;border-top:1px solid {BORDER};padding:11px 0;">
            <div style="font-family:'JetBrains Mono',monospace;font-size:13px;
    ...
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:16px;padding:22px;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.16em;color:{TXT3};margin-bottom:14px;">◈ TRADING SETUP</div>
      <table style="width:100%;border-collapse:collapse;">
        {srow("ENTRY PRICE",           cur_price, BLUE)}
        {srow("STOP LOSS  (2× ATR)",   sl,        RED)}
        {srow("TAKE PROFIT 1  (3× ATR)", tp1,     GREEN)}
        {srow("TAKE PROFIT 2  (6× ATR)", tp2,     GREEN)}
        <tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
               padding:11px 0;border-top:1px solid {BORDER};">RISK / REWARD</td>
          <td style="text-align:right;border-top:1px solid {BORDER};padding:11px 0;">
            <span style="font-family:'Outfit',sans-serif;font-size:22px;font-weight:900;
                 color:{ORANGE};">1 : {rr:.2f}</span>
          </td>
        </tr>
        <tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
               padding:11px 0;border-top:1px solid {BORDER};">ATR (14)</td>
          <td style="text-align:right;border-top:1px solid {BORDER};padding:11px 0;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:{TXT2};">
        ...
        <tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{TXT3};
               padding:11px 0;border-top:1px solid {BORDER};">SIGNAL</td>
          <td style="text-align:right;border-top:1px solid {BORDER};padding:11px 0;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;
                 color:{sig_col};background:{rgba(sig_col.lstrip('#') if sig_col.startswith('#') else sig_col, 0.12)};
                 padding:4px 14px;border-radius:6px;">{signal}</span>
          </td>
        </tr>
      </table>
      <div style="margin-top:14px;padding:12px;background:{BG3};border-radius:10px;
           border:1px solid {BORDER};">
        <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
             margin-bottom:4px;letter-spacing:0.1em;">⚠ DISCLAIMER</div>
        <div style="font-family:'Outfit',sans-serif;font-size:11px;color:{TXT3};line-height:1.6;">
...
def mtf_table_html(ts_score, cls_sig, latest):
    ...
    e50    = float(latest.get("EMA50", 0))
    e200   = float(latest.get("EMA200", 0))
    ema_str = f"20{'>' if e20>e50 else '<'}50{'>' if e50>e200 else '<'}200"
    ...
    def rhtml(tf_, sig_, ema_, rsi_):
        sc  = GREEN if sig_=="BUY" else RED if sig_=="SELL" else YELLOW
        arr = "↑" if sig_=="BUY" else "↓" if sig_=="SELL" else "↔"
        return f"""<tr>
          <td style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{TXT2};
               padding:11px 0;border-top:1px solid {BORDER};">{tf_}</td>
          <td style="text-align:center;padding:11px 0;border-top:1px solid {BORDER};">
            <span style="color:{sc};font-size:22px;">{arr}</span>
          </td>
          <td style="text-align:center;font-family:'JetBrains Mono',monospace;font-size:10px;
          ...
          <td style="text-align:right;padding:11px 0;border-top:1px solid {BORDER};">
            <span style="font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:700;
                 color:{sc};background:{rgba(sc, 0.1)};border:1px solid {rgba(sc, 0.3)};
                 padding:3px 12px;border-radius:5px;">{sig_}</span>
          </td>
        </tr>"""

    rows_html = "".join(rhtml(*r) for r in rows_data)
    return f"""
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:16px;padding:22px;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;
           letter-spacing:0.16em;color:{TXT3};margin-bottom:14px;">◈ MULTI-TIMEFRAME ANALYSIS</div>
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr>
            <th style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
...
def main():
    ...
    with st.sidebar:
        st.markdown(f"""
        <div style="padding:20px 0 20px;border-bottom:1px solid {BORDER};margin-bottom:4px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:36px;height:36px;border-radius:50%;
                 background:rgba(255,149,0,0.1);display:flex;align-items:center;
                 justify-content:center;border:1px solid rgba(255,149,0,0.3);">
            ...
            <div>
              <div style="font-family:'Outfit',sans-serif;font-size:17px;font-weight:800;
                   color:#fff;letter-spacing:-0.3px;">RENDER</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:{TXT3};
                   letter-spacing:0.12em;margin-top:1px;">AI TERMINAL</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-section">Timeframe</div>', unsafe_allow_html=True)
        tf_map = {"1 Hour": "1h", "4 Hours": "4h", "1 Day": "1d"}
        sel_tf = tf_map[st.selectbox("tf", list(tf_map.keys()), label_visibility="collapsed")]

        st.markdown(f'<div class="sidebar-section">Period</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="sidebar-section">Chart Overlays</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="sidebar-section">AI Models</div>', unsafe_allow_html=True)
        models_ok = all(os.path.exists(f) for f in MODEL_FILES)
        if models_ok:
            st.markdown(f"""
            <div style="background:{rgba(GREEN, 0.08)};border:1px solid {rgba(GREEN, 0.25)};
                 border-radius:10px;padding:10px 12px;">
              <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{GREEN};">
                ✓ ML Models loaded</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};margin-top:3px;">
                XGBoost · LightGBM · RF</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style="background:{rgba(YELLOW, 0.08)};border:1px solid {rgba(YELLOW, 0.25)};
                 border-radius:10px;padding:10px 12px;">
              <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{YELLOW};">
                ⚠ Models not found</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};margin-top:3px;">
                Run: python train.py<br>Rule-based signals active</div>
            </div>""", unsafe_allow_html=True)

        st.markdown(f'<div class="sidebar-section">Data</div>', unsafe_allow_html=True)
        if st.button("↻  Refresh Data", use_container_width=True):
            st.cache_data.clear()
            st.rerun()

        st.markdown(f"""
        <div style="margin-top:22px;padding-top:14px;border-top:1px solid {BORDER};
             font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};line-height:2;">
    ...
    <div style="background:{BG2};border-bottom:1px solid {BORDER};
         padding:16px 24px;margin:0 -20px 24px -20px;
         display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
      <div style="display:flex;align-items:center;gap:14px;">
        <div style="width:46px;height:46px;border-radius:50%;
             background:rgba(240,136,62,0.1);display:flex;align-items:center;
             justify-content:center;border:1px solid rgba(240,136,62,0.3);">
        ...
        <div>
          <div style="font-family:'Outfit',sans-serif;font-size:20px;font-weight:800;
               color:#fff;letter-spacing:-0.3px;line-height:1.1;">RENDER NETWORK · RNDR</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};
      ...
      <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
        <div>
          <div style="font-family:'Outfit',sans-serif;font-size:32px;font-weight:900;
               color:#f0f4ff;letter-spacing:-1px;line-height:1;">${cur_price:.4f}</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:{TXT2};margin-top:3px;">
            ₹{price_inr:,.2f}
          </div>
        </div>
        <div style="background:{rgba(up_col, 0.1)};border:1px solid {rgba(up_col, 0.3)};
          ...
          <div style="font-family:'JetBrains Mono',monospace;font-size:15px;font-weight:700;
               color:{up_col};">{'+'if is_up else ''}{chg24:.2f}%</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};margin-top:2px;">
            24H CHANGE</div>
        </div>
        <div style="background:{rgba(sig_col, 0.1)};border:1px solid {rgba(sig_col, 0.3)};
             padding:8px 16px;border-radius:10px;text-align:center;">
          <div style="font-family:'Outfit',sans-serif;font-size:18px;font-weight:800;
               color:{sig_col};">{cls_sig}</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{TXT3};margin-top:2px;">
            AI SIGNAL · {conf}%</div>
        </div>
        <div style="display:flex;align-items:center;gap:7px;background:{rgba(GREEN, 0.08)};
             border:1px solid {rgba(GREEN, 0.2)};padding:8px 16px;border-radius:20px;">
    ...
    # ═══════════════════════════════════════════════════
    # TECHNICAL INDICATOR CARDS (AT TOP — BEFORE CHARTS)
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("TECHNICAL INDICATORS", "Live values from latest candle"), unsafe_allow_html=True)

    rsi_v  = float(latest.get("RSI",       50))
    macd_h = float(latest.get("MACD_HIST",  0))
    adx_v  = float(latest.get("ADX",        0))
    stk_v  = float(latest.get("STOCH_K",   50))
    atr_d  = float(latest.get("ATR", cur_price * 0.02))
    ...
    def gen_sig(bull, bear, lb, ln, ls):
        if bull: return lb, GREEN
        if bear: return ls, RED
        return ln, YELLOW
    ...
    indicators = [
        ...
        ("MACD HIST",    f"{macd_h:+.4f}",     *gen_sig(macd_h>0, macd_h<0, "BULLISH","NEUTRAL","BEARISH")),
        ("ADX 14",       f"{adx_v:.1f}",        *gen_sig(adx_v>25, adx_v<15, "STRONG","MODERATE","WEAK")),
        ("STOCH %K",     f"{stk_v:.1f}",        *gen_sig(stk_v<20, stk_v>80, "OVERSOLD","NEUTRAL","OVERBOUGHT")),
        ("ATR 14",       f"${atr_d:.4f}",       "VOLATILITY", BLUE),
        ("BB WIDTH",     f"{bb_wv:.2f}%",       *gen_sig(bb_wv<2, bb_wv>8, "LOW VOL","NORMAL","HIGH VOL")),
        ("CCI 20",       f"{cci_v:.1f}",         *gen_sig(cci_v<-100, cci_v>100, "OVERSOLD","NEUTRAL","OVERBOUGHT")),
        ("MFI 14",       f"{mfi_v:.1f}",         *gen_sig(mfi_v<20, mfi_v>80, "OVERSOLD","NEUTRAL","OVERBOUGHT")),
        ("CMF 20",       f"{cmf_v:+.3f}",        *gen_sig(cmf_v>0, cmf_v<0, "INFLOW","NEUTRAL","OUTFLOW")),
        ("WILLIAMS %R",  f"{wil_v:.1f}",         *gen_sig(wil_v<-80, wil_v>-20, "OVERSOLD","NEUTRAL","OVERBOUGHT")),
        ("ROC 5",        f"{roc_v:+.2f}%",       *gen_sig(roc_v>0, roc_v<0, "POSITIVE","NEUTRAL","NEGATIVE")),
    ...
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("TRADINGVIEW LIVE CHART", "CRYPTO:RENDERUSD · 1H"), unsafe_allow_html=True)

    components.html("""
    <div style="border-radius:16px;overflow:hidden;border:1px solid #1a2035;">
      <div class="tradingview-widget-container" style="height:560px;width:100%;">
          ...
          src="https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js" async>
        {
          ...
          "style": "1",
          "locale": "en",
          "backgroundColor": "rgba(10,13,26,1)",
          "gridColor": "rgba(26,32,53,0.8)",
    ...
    # LOCAL OHLCV CHART
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("OHLCV CHART — LOCAL DATA",
    ...
    # ═══════════════════════════════════════════════════
    # INDICATOR CHARTS GRID (each chart is SEPARATE)
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("MOMENTUM INDICATOR CHARTS"), unsafe_allow_html=True)

    r1c1, r1c2 = st.columns(2)
    with r1c1: st.plotly_chart(chart_macd(df),  use_container_width=True, config=CHART_CONFIG)
    with r1c2: st.plotly_chart(chart_rsi(df),   use_container_width=True, config=CHART_CONFIG)

    r2c1, r2c2 = st.columns(2)
    with r2c1: st.plotly_chart(chart_stoch(df), use_container_width=True, config=CHART_CONFIG)
    with r2c2: st.plotly_chart(chart_adx(df),   use_container_width=True, config=CHART_CONFIG)

    r3c1, r3c2 = st.columns(2)
    with r3c1: st.plotly_chart(chart_bb(df),             use_container_width=True, config=CHART_CONFIG)
    with r3c2: st.plotly_chart(chart_cci_williams(df),   use_container_width=True, config=CHART_CONFIG)

    r4c1, r4c2 = st.columns(2)
    with r4c1: st.plotly_chart(chart_mfi(df),  use_container_width=True, config=CHART_CONFIG)
    with r4c2: st.plotly_chart(chart_roc(df),  use_container_width=True, config=CHART_CONFIG)

    # ═══════════════════════════════════════════════════
    # VOLUME & FLOW — SEPARATE CHARTS
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("VOLUME & FLOW ANALYSIS"), unsafe_allow_html=True)
    ...
    vc4 = st.columns(4)
    for col_v, (lbl, vl, sub, cc) in zip(vc4, [
    ...
    <div style="background:{BG2};border:1px solid {BORDER};border-radius:12px;
         overflow:hidden;height:34px;display:flex;">
      <div style="width:{buy_pres}%;background:{GREEN};display:flex;align-items:center;
           ...
           font-weight:700;color:#000;letter-spacing:0.05em;">BUY {buy_pres:.1f}%</div>
      <div style="flex:1;background:{RED};display:flex;align-items:center;
           justify-content:center;font-family:'JetBrains Mono',monospace;font-size:11px;
           font-weight:700;color:#fff;letter-spacing:0.05em;">SELL {sell_pres:.1f}%</div>
    </div>""", unsafe_allow_html=True)

    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)

    # Separate volume charts
    v1c1, v1c2 = st.columns(2)
    with v1c1: st.plotly_chart(chart_volume(df), use_container_width=True, config=CHART_CONFIG)
    with v1c2: st.plotly_chart(chart_obv(df),    use_container_width=True, config=CHART_CONFIG)

    st.plotly_chart(chart_cmf(df), use_container_width=True, config=CHART_CONFIG)

    # ═══════════════════════════════════════════════════
    # AI PREDICTIONS — CLASSIFICATION + REGRESSION SEPARATE
    # ═══════════════════════════════════════════════════
    st.markdown(section_header("AI PREDICTION ENGINE",
                               "Classification (direction) · Regression (price target)"), unsafe_allow_html=True)

    ai_l, ai_m, ai_r = st.columns([1.1, 1.1, 0.9])
    with ai_l:
        st.markdown(
            classification_card_html(cls_sig, cls_proba, conf, sig_col, cur_price, inr_rate),
            unsafe_allow_html=True,
        )
    with ai_m:
        st.markdown(
    ...
    with ai_r:
        st.markdown(regime_html(ts, atr_v, cur_price), unsafe_allow_html=True)
    ...
    with ts_col:
        st.markdown(trading_setup_html(cur_price, inr_rate, atr_v, cls_sig), unsafe_allow_html=True)
...
if __name__ == "__main__":
    main()import os, json, warnings, time
warnings.filterwarnings("ignore")
