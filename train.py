...
class ModelConfig:
    ...
    models_to_train:          List[ModelType] = field(default_factory=lambda: [
        ...
        ModelType.TRANSFORMER, ModelType.CNN_LSTM,
...
class DataPreprocessor:
    ...
    def build_targets(self, close: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """
        Returns:
            y_reg   – future log return (regression target)
        ...
        future_ret = close.shift(-h).pct_change(h).shift(-(h - 1))
...
def train_sklearn_model(model, X_train, y_train, X_val, y_val,
    ...
    if is_classification:
        tr_score = accuracy_score(y_train, model.predict(X_train))
        vl_score = accuracy_score(y_val,   model.predict(X_val))
        metric   = "accuracy"
    else:
